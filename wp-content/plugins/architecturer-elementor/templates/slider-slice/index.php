<?php
	...
	if(!empty($slides))
	{
		...
		{
			$height_style = 'height:'.intval($settings['height']['size']).'px;';
		}
		else
		{
			$height_style = 'height:'.intval($settings['height']['size']).'vh;';
		}
?>
<section class="tg_slice_slide_container slides" style="width:<?php echo intval($settings['width']['size']).$settings['width']['unit']; ?>;<?php echo esc_attr($height_style); ?>">
	<?php
		if($count_slides > 1)
		{
	?>
		<section class="slides-nav">
		    <nav class="slides-nav__nav">
		      	<button class="slides-nav__prev js-prev"><?php esc_html_e( 'Prev' , 'architecturer-elementor' ); ?></button>
			  	<button class="slides-nav__next js-next"><?php esc_html_e( 'Next' , 'architecturer-elementor' ); ?></button>
		    </nav>
	  	</section>
	<?php
		}
