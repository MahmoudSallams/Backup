...
?>
<div class="portfolio_timeline_vertical_content_wrapper" <?php if($settings['autoplay'] == 'yes') { ?>data-autoplay="<?php echo esc_attr($timer); ?>"<?php } ?> data-speed="<?php echo esc_attr($settings['transition_speed']['size']); ?>">
	<div class="timeline">
		<div class="swiper-container"  style="height:<?php echo intval($settings['height']['size']).$settings['height']['unit']; ?>;">
			...
			?>
					<div class="swiper-slide" style="<?php echo esc_attr($css_bg_img); ?>" data-year="<?php echo $slide['slide_label']; ?>">
				        ...
				        <a class="portfolio_timeline_vertical_link" href="<?php echo esc_url($slide['slide_link']['url']); ?>" <?php echo esc_attr($target); ?> ></a>
					</div>
			<?php
						$count++;
					}
