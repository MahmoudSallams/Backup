...
function rt_add_gravatar_col($cols) {
	$cols['thumbnail'] = esc_html__('Thumbnail', 'architecturer');
	return $cols;
}
...
$postmetas = 
	array (
		'post' => array(
			array("id" => "post_layout", "type" => "select", "title" => esc_html__('Post Layout', 'architecturer' ), "description" => esc_html__('You can select layout of this single post page.', 'architecturer' ), "items" => $post_layout_select),
			
			array("id" => "page_menu_transparent", "type" => "checkbox", "title" => esc_html__('Make Menu Transparent', 'architecturer' ), "description" => esc_html__('Check this option if you want to display main menu in transparent', 'architecturer' )),
			
			array("id" => "post_video_embed", "type" => "textarea", "title" => esc_html__('Video Embed Code', 'architecturer' ), "description" => esc_html__('Insert Youtube or Vimeo embed code.', 'architecturer' )),
		),
);

function architecturer_create_meta_box() {
	...
	if ( function_exists('add_meta_box') && isset($postmetas) && count($postmetas) > 0 ) {  
		foreach($postmetas as $key => $postmeta)
		{
			...
			{
				add_meta_box( 'metabox', ucfirst($key).' Options', 'architecturer_new_meta_box', $key, 'normal', 'high' );
			}
		}
	}
...
function architecturer_new_meta_box() {
		...
		if($_GET['post_type'] == $key)
		{
			...
			foreach ( $postmeta as $postmeta_key => $each_meta ) {
				...
				if ($meta_type == 'checkbox') {
					...
					echo '<th width="5%"></th>';
					echo '<th width="90%">'.esc_html__("Title", 'architecturer').'</th>';
					echo '<th width="5%"></th>';
					...
					echo '<a id="'.$meta_id.'_add_row" class="button adding_list_add_row" data-target="'.$meta_id.'_sortable" data-metaid="'.$meta_id.'" href="javascript:;">'.esc_html__("Add another", 'architecturer').'</a>';
...
function architecturer_save_postdata( $post_id ) {
		...
		foreach ( $postmetas as $postmeta ) {
			foreach ( $postmeta as $each_meta ) {
				...
				if (isset($_POST[$each_meta['id']]) && $_POST[$each_meta['id']]) {
					architecturer_update_custom_meta($post_id, $_POST[$each_meta['id']], $each_meta['id']);
				}
...
function architecturer_update_custom_meta($postID, $newvalue, $field_name) {
...
add_action('admin_menu', 'architecturer_create_meta_box'); 
add_action('save_post', 'architecturer_save_postdata'); 	
?>