...
**/
function architecturer_pagination($pages = '', $range = 4, $class = '')
{
	 $showitems = ($range * 2)+1;
	 
	 $paged = architecturer_get_paged();
	 if(empty($paged)) $paged = 1;
	 
	 if($pages == '')
	 {
	 $wp_query = architecturer_get_wp_query();
...
function architecturer_get_review($post_id = '', $rating_field = '')
{
	$rating_arr = array();
...
*	Setup comment style
**/
function architecturer_comment($comment, $args, $depth) 
{
		...
		<div class="gravatar">
      	...
      	<div class="right <?php if($has_avatar == '') { ?>fullwidth<?php } ?>">
			<?php if ($comment->comment_approved == '0') : ?>
         		<em><?php echo esc_html_e('(Your comment is awaiting moderation.)', 'architecturer') ?></em>
         		<br />
      		<?php endif; ?>
			...
			<div class="comment_date"><?php echo date_i18n(ARCHITECTURER_THEMEDATEFORMAT, strtotime($comment->comment_date)); ?> <?php echo esc_html_e('at', 'architecturer') ?> <?php echo date_i18n(ARCHITECTURER_THEMETIMEFORMAT, strtotime($comment->comment_date)); ?></div>
			<?php 
      			if($depth < 3)
      			{
      		?>
      			<?php comment_reply_link(array_merge( $args, array('depth' => $depth,
'reply_text' =>  __('Reply', 'architecturer'), 'login_text' => __('Login to Reply', 'architecturer'), 'max_depth' => $args['max_depth']))) ?>
			<?php
				}
			?>
...
function architecturer_substr($str, $length, $minword = 3)
{
    $sub = '';
    $len = 0;
...
**/
function architecturer_posts($sort = 'recent', $items = 3, $echo = TRUE, $show_thumb = TRUE) 
{
	$return_html = '';
	...
	if($sort == 'recent')
	{
		...
		$title = esc_html__('Recent Posts', 'architecturer');
	}
	...
	if(!empty($posts))
	{
		...
		$count_post = count($posts);
			...
			foreach($posts as $post)
			{
				...
				$return_html.= '<a href="'.get_permalink($post->ID).'">'.$post->post_title.'</a><div class="post_attribute">'.date_i18n(ARCHITECTURER_THEMEDATEFORMAT, get_the_time('U', $post->ID)).'</div>';
				$return_html.= '</li>';
...
function architecturer_cat_posts($cat_id = '', $items = 5, $echo = TRUE, $show_thumb = TRUE) 
{
	$return_html = '';
	...
	if(!empty($posts))
	{
		...
		$return_html.= '">';
			...
			foreach($posts as $key => $post)
			{
				...
				$return_html.= '<a href="'.get_permalink($post->ID).'">'.architecturer_substr($post->post_title, 50).'</a><div class="post_attribute">'.date_i18n(ARCHITECTURER_THEMEDATEFORMAT, get_the_time('U', $post->ID)).'</div>';
				$return_html.= '</li>';
			}	
...
function architecturer_image_from_description($data) {
    preg_match_all('/<img src="([^"]*)"([^>]*)>/i', $data, $matches);
    return $matches[1][0];
}


function architecturer_select_image($img, $size) {
    $img = explode('/', $img);
...
function architecturer_get_flickr($settings) 
{
	...
	$flickr_cache_path = ARCHITECTURER_THEMEUPLOAD.'/flickr_'.$settings['id'].'_'.$settings['items'].'.cache';
	...
	if(!file_exists($flickr_cache_path) OR $flickr_cache_timer > 15)
	{
		...
		foreach ($feed->get_items() as $key => $item)
		{
			...
			$img = architecturer_image_from_description($item->get_description()); 
			$thumb_url = architecturer_select_image($img, 1);
			$large_url = architecturer_select_image($img, 4);
		...
		if(!empty($photos_arr))
		{
			...
			//Writing cache file
			$wp_filesystem = architecturer_get_wp_filesystem();
...
function architecturer_get_instagram($username, $access_token, $items = 8)
{   
	$wp_filesystem = architecturer_get_wp_filesystem();
    ...
    {
	    $instagram_cache_path = ARCHITECTURER_THEMEUPLOAD.'/instagram_'.$username.'_'.$items.'.cache';
		...
		if(!file_exists($instagram_cache_path) OR $instagram_cache_timer > 15)
		{
		    ...
		    {
		    	$shots = $isg->getTagMedia(array('count'=> $items), ARCHITECTURER_THEMEDEMOIG); 
		    }
...
function architecturer_apply_content($pp_content) {
...
function architecturer_get_excerpt_by_id($post_id)
{
...
function architecturer_aasort(&$array, $key) 
{
    $sorter=array();
    $ret=array();
    reset($array);
...
function architecturer_update_urls($options,$oldurl,$newurl)
{	
	$wpdb = architecturer_get_wpdb();
	$results = array();
	$queries = array(
	'content' =>		array("UPDATE $wpdb->posts SET post_content = replace(post_content, %s, %s)",  esc_html__('Content Items (Posts, Pages, Custom Post Types, Revisions)','architecturer') ),
	'excerpts' =>		array("UPDATE $wpdb->posts SET post_excerpt = replace(post_excerpt, %s, %s)", esc_html__('Excerpts','architecturer') ),
	'attachments' =>	array("UPDATE $wpdb->posts SET guid = replace(guid, %s, %s) WHERE post_type = 'attachment'",  esc_html__('Attachments','architecturer') ),
	'links' =>			array("UPDATE $wpdb->links SET link_url = replace(link_url, %s, %s)", esc_html__('Links','architecturer') ),
	'custom' =>			array("UPDATE $wpdb->postmeta SET meta_value = replace(meta_value, %s, %s)",  esc_html__('Custom Fields','architecturer') ),
	'guids' =>			array("UPDATE $wpdb->posts SET guid = replace(guid, %s, %s)",  esc_html__('GUIDs','architecturer') )
	);
	    ...
	    if( $option == 'custom' ){
	    	...
	    	for( $page = 0; $page < $pages; $page++ ) {
	    		...
	    		if( trim($value) == '' )
	    			...
	    			$edited = architecturer_unserialize_replace( $oldurl, $newurl, $value );
...
function architecturer_unserialize_replace( $from = '', $to = '', $data = '', $serialised = false ) 
{
    try {
    	if ( is_string( $data ) && ( $unserialized = @unserialize( $data ) ) !== false ) {
    		$data = architecturer_unserialize_replace( $from, $to, $unserialized, true );
    	}
    	elseif ( is_array( $data ) ) {
    		$_tmp = array( );
    		foreach ( $data as $key => $value ) {
    			$_tmp[ $key ] = architecturer_unserialize_replace( $from, $to, $value, false );
    		}
    		$data = $_tmp;
    		unset( $_tmp );
    	}
...
function architecturer_get_first_title_word($title) {
	return $title;
}

function architecturer_menu_layout() {
	$tg_menu_layout = get_theme_mod('tg_menu_layout', 'leftalign');

	if(ARCHITECTURER_THEMEDEMO && isset($_GET['menulayout']) && !empty($_GET['menulayout']))
	{
...
/**
* architecturer_is_woocommerce_page - Returns true if on a page which uses WooCommerce templates (cart and checkout are standard pages with shortcodes and which are also included)
*
* @access public
* @return bool
*/
function architecturer_is_woocommerce_page() 
{
...
function architecturer_check_system()
{
	$has_error = 0;
...
function architecturer_hex_darker($hex,$factor = 30)
{
    $new_hex = '';
...
function architecturer_get_dynamic_sidebar($index = 1)
{
    $sidebar_contents = "";
    ob_start();
    dynamic_sidebar($index);
...
function architecturer_hex_to_rgb($hex) 
{
	$hex = str_replace("#", "", $hex);
	$color = array();
...
function architecturer_available_widgets() 
{
	$wp_registered_widget_controls = architecturer_get_registered_widget_controls();
...
function architecturer_import_data( $data ) 
{
	$wp_registered_sidebars = architecturer_get_registered_sidebars();

	// Have valid data?
	// If no data or could not decode
	if ( empty( $data ) || ! is_object( $data ) ) {
		wp_die(
			esc_html__('Import data could not be read. Please try a different file.', 'architecturer' ),
			'',
	...
	$available_widgets = architecturer_available_widgets();
	...
	foreach ( $data as $sidebar_id => $widgets ) {
		...
		if ( isset( $wp_registered_sidebars[$sidebar_id] ) ) {
			...
			$sidebar_message = esc_html__('Sidebar does not exist in theme (using Inactive)', 'architecturer' );
		}
		...
		foreach ( $widgets as $widget_instance_id => $widget ) {
			...
			if ( ! $fail && ! isset( $available_widgets[$id_base] ) ) {
				...
				$widget_message = esc_html__('Site does not support widget', 'architecturer' ); // explain why widget not imported
			}
			...
			if ( ! $fail && isset( $widget_instances[$id_base] ) ) {
				...
				foreach ( $single_widget_instances as $check_id => $check_widget ) {
					...
					if ( in_array( "$id_base-$check_id", $sidebar_widgets ) && (array) $widget == $check_widget ) {
						...
						$widget_message = esc_html__('Widget already exists', 'architecturer' ); // explain why widget not imported
			...
			if ( ! $fail ) {
				...
				if ( $sidebar_available ) {
					...
					$widget_message = esc_html__('Imported', 'architecturer' );
				} else {
					$widget_message_type = 'warning';
					$widget_message = esc_html__('Imported to Inactive', 'architecturer' );
				}
			...
			$results[$sidebar_id]['widgets'][$widget_instance_id]['title'] = ! empty( $widget['title'] ) ? $widget['title'] : esc_html__('No Title', 'architecturer' ); // show "No Title" if widget instance is untitled
...
function architecturer_count_client_images($client_id = '')
{
	if(!empty($client_id))
	{
		$images_counter = 0;
...
function architecturer_set_map_api()
{
	...
	{
		wp_enqueue_script("google_maps", "https://maps.googleapis.com/maps/api/js", false, ARCHITECTURER_THEMEVERSION, true);
	}
	else
	{
		wp_enqueue_script("google_maps", "https://maps.googleapis.com/maps/api/js?key=".$pp_googlemap_api_key, false, ARCHITECTURER_THEMEVERSION, true);
	}
}

function architecturer_live_builder_begin_wrapper($item_id = '', $item_type = '')
{
	...
	if(isset($_GET['ppb_live']) && !empty($item_id))
	{
		...
		{
			$return_html.= '<a href="javascript:;" class="ppb_add_after" data-builder-id="'.esc_attr($item_id).'" title="'.esc_html__('Add Content After This', 'architecturer' ).'"><i class="fa fa-plus"></i></a>';
		
			$return_html.= '<a href="javascript:;" class="ppb_edit" data-builder-id="'.esc_attr($item_id).'" title="'.esc_html__('Edit', 'architecturer' ).'"><i class="fa fa-edit"></i></a>';
			
			$return_html.= '<a href="javascript:;" class="ppb_duplicate" data-builder-id="'.esc_attr($item_id).'" title="'.esc_html__('Duplicate', 'architecturer' ).'"><i class="fa fa-copy"></i></a>';
		}
		
		$return_html.= '<a href="javascript:;" class="ppb_remove" data-builder-id="'.esc_attr($item_id).'" title="'.esc_html__('Remove', 'architecturer' ).'"><i class="fa fa-trash"></i></a>';
...
function architecturer_live_builder_end_wrapper($item_id = '')
{
	//Check if live content builder mode
	if(isset($_GET['ppb_live']) && !empty($item_id))
	{
...
function architecturer_get_post_view($post_id = '', $raw = false)
{
...
function architecturer_get_post_share($post_id = '', $raw = false)
{
	if($raw)
	{
...
function architecturer_get_demo_logo($logo_name = 'tg_retina_logo')
{
	$logo_url = '';
	...
	{
		case 'tg_retina_logo':
			$logo_url = get_site_url().'/wp-content/themes/architecturer/images/logo@2x.png';
		break;
		
		case 'tg_retina_transparent_logo':
			$logo_url = get_site_url().'/wp-content/themes/architecturer/images/logo@2x_white.png';
		break;
	}
...
function architecturer_get_demo_url($path = '')
{
	$demo_url = 'https://themes.themegoods.com/architecturer/demo/'.$path;
	return $demo_url;
}

function architecturer_themegoods_action() 
{
	if(defined('THEMEGOODS') && THEMEGOODS)
	{
		update_option("pp_verified_envato_architecturer", true);
		update_option("pp_envato_personal_token", '[ThemeGoods Activation]');
	}
}

function architecturer_get_footer_elementor($tg_footer_content_default = '')
{
	if (class_exists("\\Elementor\\Plugin")) {
        $pluginElementor = \Elementor\Plugin::instance();
        $contentElementor = $pluginElementor->frontend->get_builder_content($tg_footer_content_default);
...
function architecturer_starts_with($haystack, $needle) 
{
    // search backwards starting from haystack length characters from the end
    return $needle === "" || strrpos($haystack, $needle, -strlen($haystack)) !== FALSE;
}

function architecturer_post_has( $type, $post_id ) 
{
...
function architecturer_write_log($log)  
{
    if (true === WP_DEBUG) 
    {
