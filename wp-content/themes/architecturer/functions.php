<?php
/*
Theme Name: Architecturer Theme
...
function architecturer_add_admin() 
{
	...
	if ( isset($_GET['page']) && $_GET['page'] == 'functions.php' ) {
		...
		if ( isset($_REQUEST['action']) && 'save' == $_REQUEST['action'] ) {
			...
			if (!wp_verify_nonce($retrieved_nonce, 'architecturer_save_theme_setting' ) ) die();
			...
			if(isset($_REQUEST['pp_envato_personal_token']) && !empty($_REQUEST['pp_envato_personal_token']) && $_REQUEST['pp_envato_personal_token'] != '[ThemeGoods Activation]')
			{
				...
				{
					$is_verified_envato_purchase_code = false;
					delete_option("pp_verified_envato_architecturer", true);
				}
			...
			foreach ($architecturer_options as $value) 
			{
			...
			foreach ($architecturer_options as $value) {
				...
				{ 
					...
					if($value['id'] != ARCHITECTURER_SHORTNAME."_sidebar0" && $value['id'] != ARCHITECTURER_SHORTNAME."_ggfont0")
					{
						//if sortable type
					...
					{
						if($value['id'] == ARCHITECTURER_SHORTNAME."_sidebar0")
						{
							//get last sidebar serialize array
							$current_sidebar = get_option(ARCHITECTURER_SHORTNAME."_sidebar");
							$request_value = $_REQUEST[ $value['id'] ];
							$request_value = sanitize_text_field($request_value);
							
							$current_sidebar[ $request_value ] = $request_value;
				
							update_option( ARCHITECTURER_SHORTNAME."_sidebar", $current_sidebar );
						}
						elseif($value['id'] == ARCHITECTURER_SHORTNAME."_ggfont0")
						{
							//get last ggfonts serialize array
							$current_ggfont = get_option(ARCHITECTURER_SHORTNAME."_ggfont");
							$current_ggfont[ $_REQUEST[ $value['id'] ] ] = $_REQUEST[ $value['id'] ];
				
							update_option( ARCHITECTURER_SHORTNAME."_ggfont", $current_ggfont );
						}
					}
				} 
				else 
	...
	add_theme_page('Theme Setting', 'Theme Setting', 'administrator', 'functions.php', 'architecturer_admin', '');
}

function architecturer_enqueue_admin_page_scripts() 
{
	$current_screen = architecturer_get_current_screen();
	
	//Enqueue CSS scripts for backend
	wp_enqueue_style('thickbox');
	wp_enqueue_style('architecturer-functions', get_template_directory_uri().'/functions/css/functions.css', false, ARCHITECTURER_THEMEVERSION, 'all');

	wp_enqueue_style('switchery', get_template_directory_uri().'/functions/css/switchery.css', false, ARCHITECTURER_THEMEVERSION, 'all');
	wp_enqueue_style("fontawesome", get_template_directory_uri()."/css/font-awesome.min.css", false, ARCHITECTURER_THEMEVERSION, "all");
	...
	wp_enqueue_script('switchery', get_template_directory_uri().'/functions/switchery.js', false, ARCHITECTURER_THEMEVERSION);
	
	wp_register_script('architecturer-theme-script', get_template_directory_uri().'/functions/theme_script.js', false, ARCHITECTURER_THEMEVERSION, true);
	$params = array(
	  'ajaxurl' => esc_url(admin_url('admin-ajax.php')),
	);
	wp_localize_script( 'architecturer-theme-script', 'tgAjax', $params );
	wp_enqueue_script( 'architecturer-theme-script' );
}

add_action('admin_enqueue_scripts',	'architecturer_enqueue_admin_page_scripts' );

function architecturer_enqueue_front_page_scripts() 
{
	wp_enqueue_style("architecturer-reset-css", get_template_directory_uri()."/css/core/reset.css", false, "");
	wp_enqueue_style("architecturer-wordpress-css", get_template_directory_uri()."/css/core/wordpress.css", false, "");
	wp_enqueue_style("architecturer-screen", get_template_directory_uri().'/css/core/screen.css', false, "", "all");
	...
	{
		case 'leftalign':
			wp_enqueue_style("architecturer-leftalignmenu", get_template_directory_uri().'/css/menus/leftalignmenu.css', false, "", "all");
		break;
		
		case 'hammenufull':
			wp_enqueue_style("architecturer-hammenufull", get_template_directory_uri().'/css/menus/hammenufull.css', false, "", "all");
		break;
		
		case 'centeralogo':
			wp_enqueue_style("architecturer-centeralogo", get_template_directory_uri().'/css/menus/centeralogo.css', false, "", "all");
		break;
	}
	
	//Add Font Awesome Support
	wp_enqueue_style("fontawesome", get_template_directory_uri()."/css/font-awesome.min.css", false, "", "all");
	wp_enqueue_style("themify-icons", get_template_directory_uri()."/css/themify-icons.css", false, ARCHITECTURER_THEMEVERSION, "all");
    
    $tg_frame = get_theme_mod('tg_frame', false);
    if(ARCHITECTURER_THEMEDEMO && isset($_GET['frame']) && !empty($_GET['frame']))
    {
	    $tg_frame = 1;
    }
    ...
    if(!empty($tg_frame))
    {
    	wp_enqueue_style("tg_frame", get_template_directory_uri()."/css/core/frame.css", false, ARCHITECTURER_THEMEVERSION, "all");
    }
	
	if(ARCHITECTURER_THEMEDEMO)
    {
	   	wp_enqueue_style('tooltipster', get_stylesheet_directory_uri()."/css/tooltipster.css", false, "", "all");
	    wp_enqueue_style('architecturer-demo', get_stylesheet_directory_uri()."/css/core/demo.css", false, "", "all");
	}
	
	//If using child theme
	if(is_child_theme())
	{
	    wp_enqueue_style('architecturer-childtheme', get_stylesheet_directory_uri()."/style.css", false, "", "all");
	}
	
	//Enqueue javascripts
	wp_enqueue_script('jquery-ui-core');
	wp_enqueue_script("jquery-effects-core");
	wp_enqueue_script("waypoints", get_template_directory_uri()."/js/waypoints.min.js", false, ARCHITECTURER_THEMEVERSION, true);
	wp_enqueue_script("tilt", get_template_directory_uri()."/js/tilt.jquery.js", false, ARCHITECTURER_THEMEVERSION, true);
	wp_enqueue_script("stellar", get_template_directory_uri()."/js/jquery.stellar.min.js", false, ARCHITECTURER_THEMEVERSION, true);
	wp_enqueue_script('modulobox', get_template_directory_uri()."/js/modulobox.js", array(), false, true );
	wp_enqueue_script("architecturer-custom-plugins", get_template_directory_uri()."/js/core/custom_plugins.js", false, ARCHITECTURER_THEMEVERSION, true);
	wp_enqueue_script("architecturer-custom-script", get_template_directory_uri()."/js/core/custom.js", false, ARCHITECTURER_THEMEVERSION, true);
	...
	if(!empty($tg_enable_right_click))
	{
		...
		wp_add_inline_script( 'architecturer-custom-script', $custom_right_click_script );
	}
	...
	if(!empty($tg_enable_dragging))
	{
		...
		wp_add_inline_script( 'architecturer-custom-script', $custom_drag_script );
	}
	...
	{
		wp_enqueue_script("sticky-kit", get_template_directory_uri()."/js/jquery.sticky-kit.min.js", false, ARCHITECTURER_THEMEVERSION, true);
	...
	{
		wp_enqueue_script("lazy", get_template_directory_uri()."/js/jquery.lazy.js", false, ARCHITECTURER_THEMEVERSION, true);
		$custom_lazy_script = '
		jQuery(function( $ ) {
	...
	if(ARCHITECTURER_THEMEDEMO)
	{
		wp_enqueue_script("tooltipster", get_template_directory_uri()."/js/jquery.tooltipster.min.js", false, ARCHITECTURER_THEMEVERSION, true);
...
add_action( 'wp_enqueue_scripts', 'architecturer_enqueue_front_page_scripts' );


//Enqueue mobile CSS after all others CSS load
function architecturer_register_mobile_css() 
{
	//Check if enable responsive layout
	wp_enqueue_style('architecturer-script-responsive-css', get_template_directory_uri()."/css/core/responsive.css", false, "", "all");
	...
	';
	wp_add_inline_style('architecturer-script-responsive-css', $animation_custom_css );
}
add_action('wp_enqueue_scripts', 'architecturer_register_mobile_css', 99);


function architecturer_admin() 
{ 
	$architecturer_options = architecturer_get_options();
	$i=0;
	...
	?>
		...
		<div class="header_wrap">
			...
			<?php
				...
				{
			?>
			<h2><?php esc_html_e('Theme Setting', 'architecturer' ); ?><span class="pp_version"><?php esc_html_e('Version', 'architecturer' ); ?> <?php echo ARCHITECTURER_THEMEVERSION; ?></span></h2>
			<?php
				}
			...
			<?php
			    	...
			    	$image_id = architecturer_get_image_id($tg_retina_logo);
			    	if(!empty($image_id))
			    	{
		    	...
		    	?>
		    	<span class="pp_version"><?php esc_html_e('Version', 'architecturer' ); ?> <?php echo ARCHITECTURER_THEMEVERSION; ?></span>
			</div>
			<?php
				}
			?>
			...
			<div style="float:right;margin:32px 0 0 0">
				<input id="save_ppsettings" name="save_ppsettings" class="button button-primary button-large" type="submit" value="<?php esc_html_e('Save', 'architecturer' ); ?>" />
				<br/><br/>
				<input type="hidden" name="action" value="save" />
				<input type="hidden" name="current_tab" id="current_tab" value="#pp_panel_general" />
				<?php wp_nonce_field('architecturer_save_theme_setting'); ?>
			</div>
	...
	?>
		...
		<a class="button pp_sortable_button" data-rel="<?php echo esc_attr($value['id']); ?>" class="button" style="display:inline-block"><?php echo esc_html__('Add', 'architecturer' ); ?></a>
		<?php
		}
		?>
...
add_action('admin_menu', 'architecturer_add_admin');
...
**/
add_action('wp_ajax_architecturer_import_demo_content', 'architecturer_import_demo_content');

function architecturer_import_demo_content() {
	$retrieved_nonce = $_REQUEST['_wpnonce'];
	if (!wp_verify_nonce($retrieved_nonce, 'architecturer_import_demo_content' ) ) die();
	...
	if(is_admin() && isset($_POST['demo']) && !empty($_POST['demo']))
	{
	    ...
	    { 
	    	...
	    	$page_on_front = 1824;
		    $oldurl = 'https://themes.themegoods.com/architecturer/demo';
		...
		if(file_exists($import_widget_filepath))
		{
			...
			$widget_import_results = architecturer_import_data( $data );
		}
		...
		architecturer_update_urls($update_options, $oldurl, $newurl);
...
if(ARCHITECTURER_THEMEDEMO)
{
	function architecturer_add_my_query_var( $link ) 
	{
		$arr_params = array();
	...
	add_filter('category_link','architecturer_add_my_query_var');
	add_filter('page_link','architecturer_add_my_query_var');
	add_filter('post_link','architecturer_add_my_query_var');
	add_filter('term_link','architecturer_add_my_query_var');
	add_filter('tag_link','architecturer_add_my_query_var');
	add_filter('category_link','architecturer_add_my_query_var');
	add_filter('post_type_link','architecturer_add_my_query_var');
	add_filter('attachment_link','architecturer_add_my_query_var');
	add_filter('year_link','architecturer_add_my_query_var');
	add_filter('month_link','architecturer_add_my_query_var');
	add_filter('day_link','architecturer_add_my_query_var');
	add_filter('search_link','architecturer_add_my_query_var');
	add_filter('previous_post_link','architecturer_add_my_query_var');
	add_filter('next_post_link','architecturer_add_my_query_var');
}
