...
<?php
	$tg_footer_content = get_theme_mod('tg_footer_content', 'content');
	$tg_footer_sidebar = get_theme_mod('tg_footer_sidebar', 4);
	
	//Check if blank template
	$architecturer_is_no_header = architecturer_get_is_no_header();
	$architecturer_screen_class = architecturer_get_screen_class();
	
	if(!is_bool($architecturer_is_no_header) OR !$architecturer_is_no_header)
	{

	$architecturer_homepage_style = architecturer_get_homepage_style();
?>
	...
	<div class="standard_wrapper">
		<?php 
			if (class_exists("\\Elementor\\Plugin")) {
                echo architecturer_get_footer_elementor($tg_footer_content_default);
            }
		?>
	</div>
<?php
...
?>
<div id="footer" class="<?php if(isset($architecturer_homepage_style) && !empty($architecturer_homepage_style)) { echo esc_attr($architecturer_homepage_style); } ?> <?php if(!empty($architecturer_screen_class)) { echo esc_attr($architecturer_screen_class); } ?>">
<?php
...
{
	//Get Footer Sidebar
	if(ARCHITECTURER_THEMEDEMO && isset($_GET['footer']) && !empty($_GET['footer']))
	{
	    $tg_footer_sidebar = 0;
	}
?>
<div class="footer_bar <?php if(isset($architecturer_homepage_style) && !empty($architecturer_homepage_style)) { echo esc_attr($architecturer_homepage_style); } ?> <?php if(!empty($architecturer_screen_class)) { echo esc_attr($architecturer_screen_class); } ?> <?php if(empty($tg_footer_sidebar)) { ?>noborder<?php } ?>">

	<div class="footer_bar_wrapper <?php if(isset($architecturer_homepage_style) && !empty($architecturer_homepage_style)) { echo esc_attr($architecturer_homepage_style); } ?>">
		<?php
			...
			{
				if($architecturer_homepage_style!='flow' && $architecturer_homepage_style!='fullscreen' && $architecturer_homepage_style!='carousel' && $architecturer_homepage_style!='flip' && $architecturer_homepage_style!='fullscreen_video')
				{	
...
?>
	    	...
	    	<?php
	    		$customizer_styling_arr = array( 
					array(
						'id'	=>	1, 
						'title' => 'Home 1', 
						'url' => architecturer_get_demo_url('home-1'),
					),
					array(
						'id'	=>	2, 
						'title' => 'Home 2', 
						'url' => architecturer_get_demo_url('home-2'),
					),
					array(
						'id'	=>	3, 
						'title' => 'Home 3', 
						'url' => architecturer_get_demo_url('home-3'),
					),
					array(
						'id'	=>	4, 
						'title' => 'Home 4', 
						'url' => architecturer_get_demo_url('home-4'),
					),
					array(
						'id'	=>	5, 
						'title' => 'Home 5', 
						'url' => architecturer_get_demo_url('home-5'),
					),
					array(
						'id'	=>	6, 
						'title' => 'Home 6', 
						'url' => architecturer_get_demo_url('home-6'),
					),
					array(
						'id'	=>	7, 
						'title' => 'Home 7', 
						'url' => architecturer_get_demo_url('home-7'),
					),
					array(
						'id'	=>	8, 
						'title' => 'Home 8', 
						'url' => architecturer_get_demo_url('home-8'),
					),
					array(
						'id'	=>	9, 
						'title' => 'Home 9', 
						'url' => architecturer_get_demo_url('home-9'),
					),
					array(
						'id'	=>	10, 
						'title' => 'Home 10', 
						'url' => architecturer_get_demo_url('home-10'),
					),
					array(
						'id'	=>	11, 
						'title' => 'Home 11', 
						'url' => architecturer_get_demo_url('home-11'),
					),
					array(
						'id'	=>	12, 
						'title' => 'Home 12', 
						'url' => architecturer_get_demo_url('home-12'),
					),
				);
	    	?>
	    	<ul class="demo_list">
    ...
    <div id="option_btn">
    	...
    	<a href="http://docs.themegoods.com/docs/architecturer/" class="demotip" title="Theme Documentation" target="_blank"><span class="ti-book"></span></a>
    	
    	<a href="https://themeforest.net/item/architecturer-interior-design-architecture-wordpress/22544684?ref=ThemeGoods&license=regular&open_purchase_for_item_id=22544684&purchasable=source&ref=ThemeGoods" title="Purchase Theme" class="demotip" target="_blank"><span class="ti-shopping-cart"></span></a>
    </div>
<?php
    	wp_enqueue_script("architecturer-jquery-cookie", get_template_directory_uri()."/js/jquery.cookie.js", false, ARCHITECTURER_THEMEVERSION, true);
    	...
    	wp_enqueue_script("architecturer-demo", get_template_directory_uri()."/js/core/demo.js", false, ARCHITECTURER_THEMEVERSION, true);
    }
?>
