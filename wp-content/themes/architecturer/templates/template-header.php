...
elseif(is_archive())
{
    ...
    elseif ( !empty($term) ) : 
    	...
    	$page_title = esc_html__('Blog Archives', 'architecturer'); 
    endif;
...
{
	$page_show_title = 0;
	$page_title = esc_html__('Search', 'architecturer' );
	$term = 'search';
}
else if(is_home())
{
	$page_show_title = 0;
	$page_title = esc_html__('Blog', 'architecturer' );
	$term = 'home';
}

$architecturer_page_content_class = architecturer_get_page_content_class();

$architecturer_hide_title = architecturer_get_hide_title();
if($architecturer_hide_title == 1)
{
	$page_show_title = 1;
}

$architecturer_screen_class = architecturer_get_screen_class();
if($architecturer_screen_class == 'split' OR $architecturer_screen_class == 'single_client')
{
	$page_show_title = 0;
}
if($architecturer_screen_class == 'single_client')
{
	$page_show_title = 1;
}
...
if(empty($page_show_title))
{
	...
	{
		$page_tagline = esc_html__('Archive posts in ', 'architecturer' );
	...
	if(is_search())
	{
		$page_tagline = esc_html__('Search Results for ', 'architecturer' ).get_search_query();
	}
	...
	if(has_post_thumbnail($current_page_id, 'full') && empty($term))
    {
		...
		{
			wp_enqueue_script("jarallax", get_template_directory_uri()."/js/jarallax.js", false, ARCHITECTURER_THEMEVERSION, true);
			wp_enqueue_script("jarallax-video", get_template_directory_uri()."/js/jarallax-video.js", false, ARCHITECTURER_THEMEVERSION, true);
			wp_enqueue_script("jarallax-element", get_template_directory_uri()."/js/jarallax-element.js", false, ARCHITECTURER_THEMEVERSION, true);
...
?>
<div id="page_caption" class="<?php if(!empty($pp_page_bg) OR $page_header_type == 'Youtube Video' OR $page_header_type == 'Vimeo Video') { ?>hasbg <?php if(!empty($tg_page_header_bg_parallax)) { ?>parallax<?php } ?> <?php } ?> <?php if(!empty($architecturer_topbar)) { ?>withtopbar<?php } ?> <?php if(!empty($architecturer_screen_class)) { echo esc_attr($architecturer_screen_class); } ?> <?php if(!empty($architecturer_page_content_class)) { echo esc_attr($architecturer_page_content_class); } ?>" <?php if(!empty($pp_page_bg)) { ?>style="background-image:url(<?php echo esc_url($pp_page_bg); ?>);"<?php } ?> <?php if($page_header_type == 'Youtube Video' OR $page_header_type == 'Vimeo Video') { ?>data-jarallax-video="<?php echo esc_url($video_url); ?>"<?php } ?>>
	...
	<div class="page_title_wrapper">
		<div class="standard_wrapper">
			<div class="page_title_inner">
				...
				<a href="<?php echo get_site_url().'/portfolio-classic/'?>" > Back To Portfolio »</a> 

 <br/>
 <br/>

					<h1 <?php if(!empty($pp_page_bg) && !empty($architecturer_topbar)) { ?>class ="withtopbar"<?php } ?>><?php echo esc_html($page_title); ?></h1>
...
<!-- Begin content -->
<div id="page_content_wrapper" class="<?php if(!empty($page_boxed_layout)) { ?>blog_wrapper <?php } ?><?php if(!empty($pp_page_bg)) { ?>hasbg <?php } ?><?php if(!empty($pp_page_bg) && !empty($architecturer_topbar)) { ?>withtopbar <?php } ?><?php if(!empty($architecturer_page_content_class)) { echo esc_attr($architecturer_page_content_class); } ?>">