...
<?php
    ...
    if(ARCHITECTURER_THEMEDEMO && isset($_GET['topbar']) && !empty($_GET['topbar']))
	{
	    $tg_topbar = true;
	}
    
    $architecturer_topbar = architecturer_get_topbar();
    architecturer_set_topbar($tg_topbar);
...
<?php
   ...
   if(!empty($pp_page_bg) && basename($pp_page_bg)=='default.png')
	...
	if(class_exists('Woocommerce') && architecturer_is_woocommerce_page())
	{
	...
	if($architecturer_homepage_style == 'fullscreen')
	{
...
<div class="top_bar <?php if(!empty($page_menu_transparent)) { ?>hasbg<?php } ?>">
    <div class="standard_wrapper">
    	...
    	<?php
    	    //get custom logo
    	    $tg_retina_logo = get_theme_mod('tg_retina_logo', architecturer_get_demo_logo('tg_retina_logo'));
    		...
    		<div class="logo_align">
	    	    <a id="custom_logo" class="logo_wrapper <?php if(!empty($page_menu_transparent)) { ?>hidden<?php } else { ?>default<?php } ?>" href="<?php echo esc_url(home_url('/')); ?>">
	    	    	<?php
						...
						{
					?>
					<img src="<?php echo esc_url($tg_retina_logo); ?>" alt="<?php esc_attr(get_bloginfo('name')); ?>" width="290" height="72"/>
					<?php
						}
						else if($image_width == 1 && $image_height == 1 && $tg_retina_logo != architecturer_get_demo_logo('tg_retina_logo'))
						{
					?>
	    	    	...
	    	    	<?php 
						...
						{
					?>
	    	    	<img src="<?php echo esc_url($tg_retina_logo); ?>" alt="<?php esc_attr(get_bloginfo('name')); ?>" width="173" height="37"/>
	    	    	<?php 
		    	    	}
		    	    ?>
	    	    </a>
    	...
    	<?php
    	    ...
    	    $tg_retina_transparent_logo = get_theme_mod('tg_retina_transparent_logo', architecturer_get_demo_logo('tg_retina_transparent_logo'));

    	    if(!empty($tg_retina_transparent_logo))
    	    {
    	    	//Get image width and height
		    	$image_id = architecturer_get_image_id($tg_retina_transparent_logo);
		    	...
		    	if(!empty($image_id) && is_numeric($image_id))
		    	{
			    	...
			    	if(isset($obj_image[1]))
			    	{
			    		$image_width = intval($obj_image[1]/1);
			    	}
			    	if(isset($obj_image[2]))
			    	{
			    		$image_height = intval($obj_image[2]/1);
			    	}
			    }
			    else if(!is_numeric($image_id))
			    {
    		...
    		<div class="logo_align">
	    	    <a id="custom_logo_transparent" class="logo_wrapper <?php if(empty($page_menu_transparent)) { ?>hidden<?php } else { ?>default<?php } ?>" href="<?php echo esc_url(home_url('/')); ?>">
	    	    	<?php
						...
						{
					?>
					<img src="<?php echo esc_url($tg_retina_transparent_logo); ?>" alt="<?php esc_attr(get_bloginfo('name')); ?>" width="290" height="<?php echo esc_attr($image_height); ?>"/>
					<?php
						}
						else if($image_width == 1 && $image_height == 1)
						{
					?>
	    	    	<img src="<?php echo esc_url($tg_retina_transparent_logo); ?>" alt="<?php esc_attr(get_bloginfo('name')); ?>" style="width="290";height:auto;"/>
	    	    	<?php 
		    	    	}
						else
						{
	    						...
	    						if ( has_nav_menu( 'primary-menu' ) ) 
	    						{
	    		    			    wp_nav_menu( 
	    		    			        	array( 
	    		    			        		...
	    		    			        		'walker' => new architecturer_walker(),
	    		    			        	) 
	    		    			    ); 
	    		    			}
	    	    			}
        ...
        <div id="logo_right_wrapper">
			...
			<div class="header_client_wrapper">
			    <a class="client_login_link" href="javascript:;" title="<?php esc_html_e('Client Login', 'architecturer' ); ?>"><span class="ti-lock"></span><?php esc_html_e('Client Login', 'architecturer' ); ?></a>
			</div>
			<?php
				}
			...
			?>
			<div class="header_client_wrapper">
			    <span class="ti-user"></span><a href="<?php echo esc_url($user_home_url); ?>"><?php echo esc_html($current_user->display_name); ?></a>&nbsp;(<a class="client_logout_link" href="<?php echo wp_logout_url( get_permalink() ); ?>" title="<?php esc_html_e('Logout', 'architecturer' ); ?>"><?php esc_html_e('Logout', 'architecturer' ); ?></a>)
			</div>
			<?php
				}
			?>
			...
			<?php
			    ...
			    $tg_menu_show_cart = get_theme_mod('tg_menu_show_cart');
			   
				if (class_exists('Woocommerce') && !empty($tg_menu_show_cart)) {
			    //Check if display cart in header
			
			    $woocommerce = architecturer_get_woocommerce();
			    $cart_url = wc_get_cart_url();
			    $cart_count = $woocommerce->cart->cart_contents_count;
			?>
			<div class="header_cart_wrapper">
			    <div class="cart_count"><?php echo esc_html($cart_count); ?></div>
			    <a href="<?php echo esc_url($cart_url); ?>" title="<?php esc_html_e('View Cart', 'architecturer' ); ?>"><span class="ti-shopping-cart"></span></a>
			</div>
			<?php
			}
			?>
